...
from utils.queries import QUERY_CAMPAIGN_TASK_COMPLETE, QUERY_CAMPAIGN_TASK_VERIFICATION, QUERY_GET_CAMPAIGN_ID, QUERY_GET_TASK_BYID, QUERY_GET_TASK_LIST, QUERY_SLOT, QUERY_USER, QUERY_LOGIN, MUTATION_GAME_PROCESS_TAPS_BATCH, QUERY_BOOSTER, QUERY_NEXT_BOSS
        ...
        if current_health == 0:
            ...
            for _ in range(50):
                ...
                if tap_result is not None:
                    if 'errors'in tap_result:
                        break
                    elif 'data' in tap_result and 'telegramGameProcessTapsBatch' in tap_result['data']:
...
def check_and_complete_tasks(index, headers):
    ...
    headers = headers_set.copy()  # Membuat salinan headers_set agar tidak mengubah variabel global
    headers['Authorization'] = f'Bearer {access_token}'
    payload = [{"operationName":"QueryVideoAdTask",
                "variables":{},
    ...
    print(f"\n[ Akun {index + 1} ]\nList Task:\n")
    datas = tasks['data']['campaignTasks']['normal']
    for task in datas:
    ...
    updated_tasks = safe_post(url, headers, payload)
...
def play_slotmachine(index):
    ...
    try:
        ...
        response =  safe_post(url,headers, json=json_data)[0]
        play_data = response.get('data', {}).get('slotMachineSpinV2', {})

        return play_data
    except Exception as error:
        print(f" | ❗️ Unknown error when Play Casino: {error}")
        return {}
    ...
    while True:
        ...
        for index, result, first_name, last_name, league in accounts:
            ...
            if cek_task_enable == 'y':
                data_campaign = get_task(index, headers=headers)
                for task in data_campaign:
                    campaigns_id = task['id']
                    if data_campaign is not None:
                        tasks_list = get_tasks_list(index, campaigns_id=campaigns_id)
                        list_task = tasks_list[0]
                        campaigns = list_task.get('data',{}).get('campaignTasks',[])
                        for campaign in campaigns:
                            name = campaign['name']
                            status = campaign['status']
                            print(f"Video: <r>{name}</r> | Status: <y>{status}</y>")
                            task_id = campaign['id']
                            time.sleep(1)
                            if status == 'Verification':
                                print(f"| Unable to complete a task, it is already in progress")
                                get_task_by_ids = get_task_by_id(index, task_id=task_id)[0]
                                data = get_task_by_ids.get('data',{})
                                campaignTaskGetConfig = data.get('campaignTaskGetConfig',{})
                                verificationAvailableAt = campaignTaskGetConfig.get('verificationAvailableAt')
                                user_task_id = campaignTaskGetConfig['userTaskId']
                                status = campaignTaskGetConfig['status']
                                # sleep_time_task = max((parser.isoparse(verificationAvailableAt) - datetime.now(
                                #     timezone.utc)).total_seconds() + 5, random.randint(5, 15))
                                # print( f"| Video: <r>{name}</r> | Status: <y>{status}</y>")
                                # print(f"Waiting {sleep_time_task}s")
                                # time.sleep(sleep_time_task)
                                if get_task_by_id is not None:
                                    complete_tasks = complete_task(index, user_task_id=user_task_id)
                                    if complete_tasks is not None:
                                        status = complete_tasks['status']
                                        print(        f"| Video: <r>{name}</r> | Status: <g>{status}</g>")
                                        time.sleep(3)
                                
                            if campaign is not None and status != 'Verification':
                                time.sleep(2)
                                verify_campaign = verify_campaigns(index, task_id=task_id)
                                ...
                                if verify_campaign is not None:
                                    ...
                                    if get_task_by_id is not None:
                                        ...
                                        if complete_tasks is not None:
                                            ...
                                            print(        f"| Video: <r>{name}</r> | Status: <g>{status}</g>")
                                            time.sleep(3)
                        data_getcampaign_id = getcampaign_id(index, campaign_id=campaigns_id)
                        if data_getcampaign_id is not None:
                            completionReward = data_getcampaign_id.get('completionReward',{})
                            id = completionReward.get('id',"")
                            data_claim_reward = claim_complete_reward(index=index, reward_id=id)
                            if data_claim_reward is not None:
                                print(f"| Reward: <g>Claimed</g>")
...
def get_task(index, headers):
    ...
    print(f"\n[ Akun {index + 1} ]\nList Task:\n")
    return tasks.get('data', {}).get('campaignLists', {}).get('normal', [])
...
def get_tasks_list(index, campaigns_id: str):
    access_token = fetch(index + 1)
    url = "api-gw-tg.memefi.club"
    headers = headers_set.copy()
    headers['Authorization'] = f'Bearer {access_token}'
    try:
        json_data = [
            ...
            'query': QUERY_GET_TASK_LIST,
            'variables': {'campaignId': campaigns_id}
        }]

        response = safe_post(url, headers, json_data)
        return response

    except Exception as e:
        print(f" | Unknown error while getting tasks: {str(e)}")
        return None

def verify_campaigns(index, task_id: str):
    access_token = fetch(index + 1)
    url = "api-gw-tg.memefi.club"
    headers = headers_set.copy()
    headers['Authorization'] = f'Bearer {access_token}'
    try:
        json_data = {
            'operationName': "CampaignTaskToVerification",
            'query': QUERY_CAMPAIGN_TASK_VERIFICATION,
        ...
        if 'errors' in data:
            print(f" | Error while verifying task: {data['errors'][0]['message']}")
            return None

        return data.get('data', {}).get('campaignTaskMoveToVerificationV2')
    except Exception as e:
        print(f" | Unknown error while verifying task: {str(e)}")
        return None

def get_task_by_id(index,task_id: str):
    access_token = fetch(index + 1)
    url = "api-gw-tg.memefi.club"
    headers = headers_set.copy()
    headers['Authorization'] = f'Bearer {access_token}'
    try:
        ...
        print(f"| Unknown error while getting task by id: {str(e)}")
        return None

def complete_task(index, user_task_id: str):
    access_token = fetch(index + 1)
    url = "api-gw-tg.memefi.club"
    headers = headers_set.copy()
    headers['Authorization'] = f'Bearer {access_token}'
    try:
        json_data = [{
            ...
            'query': QUERY_CAMPAIGN_TASK_COMPLETE,
        ...
        if 'errors' in data:
            print(f" Error while completing task: {data['errors'][0]['message']}")
            return None

        return data.get('data', {}).get('campaignTaskMarkAsCompleted')

    except Exception as e:
        print(f"Unknown error while completing task: {str(e)}")
        return None

def getcampaign_id(index, campaign_id):
    access_token = fetch(index + 1)
    url = "api-gw-tg.memefi.club"
    headers = headers_set.copy()
    headers['Authorization'] = f'Bearer {access_token}'
    payload = [{'operationName': "GetCampaignById",
            'query': QUERY_GET_CAMPAIGN_ID,
    ...
    try:
        ...
        if 'errors' in data:
            print(f" Error while completing task: {data['errors'][0]['message']}")
            return None
        return data.get('data', {}).get('campaignGetById')
    except Exception as e:
        print(f"Unknown error while completing task: {str(e)}")
        return None

def claim_complete_reward(index, reward_id):
    access_token = fetch(index + 1)
    url = "api-gw-tg.memefi.club"
    headers = headers_set.copy()
    headers['Authorization'] = f'Bearer {access_token}'
    payload = [{"operationName":"CampaignsClaimCompletionReward",
                "variables":{"rewardId":reward_id},
                "query":"""mutation CampaignsClaimCompletionReward($rewardId: String!) {
                    campaignsClaimCompletionReward(rewardId: $rewardId)
                    }"""}]
    try:
        response =  safe_post(url, headers, payload)
        data =  response[0]
        if 'errors' in data:
            print(f" Error while completing task: {data['errors'][0]['message']}")
            return None
        return data.get('data', {}).get('campaignsClaimCompletionReward')
    except Exception as e:
        print(f"Unknown error while completing task: {str(e)}")
        return None
